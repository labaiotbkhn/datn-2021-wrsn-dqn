...
class Network:
    ...
    def run_per_second(self, t, optimizer, deep_optimizer):
        ...
        if optimizer:
            self.mc.run(network=self, time_stem=t, net=self, optimizer=optimizer,  deep_optimizer=deep_optimizer)
        return state
    ...
    def simulate_max_time(self, optimizer, deep_optimizer, max_time=10000, file_name="log/information_log.csv"):
        ...
        while t <= max_time:
            ...
            if current_dead != nb_dead or current_package != nb_package:
                ...
                print("time: {}, nb dead: {}, nb package:{}".format(t, nb_dead, nb_package))
        print(t, self.mc.current, self.node[self.find_min_node()].energy)
    ...
    def simulate(self, optimizer,deep_optimizer, max_time=None, file_name="log/energy_log.csv"):
        if max_time:
            life_time = self.simulate_max_time(optimizer=optimizer, deep_optimizer=deep_optimizer, max_time=max_time, file_name=file_name)
        else:
            life_time = self.simulate_lifetime(optimizer=optimizer,deep_optimizer=deep_optimizer, file_name=file_name)
        return life_time
